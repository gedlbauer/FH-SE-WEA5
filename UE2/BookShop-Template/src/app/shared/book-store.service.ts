...
export class BookStoreService {
  ...
  getAll(): Observable<Book[]> {
    return this.http.get<any>(`${environment.server}/books`).pipe(map(res =>res['books']), catchError(this.errorHandler));
  }

  getBookById(id: string): Observable<Book> {
    return this.http.get<any>(`${environment.server}/book/${id}`).pipe(map(x => x['book']), catchError(this.errorHandler));
  }

  save(book: Book): Observable<any>{
    return this.http.post<any>(`${environment.server}/save`, book).pipe(catchError(this.errorHandler));
  }

  update(book: Book):Observable<any> {
    return this.http.put<any>(`${environment.server}/update`, book).pipe(catchError(this.errorHandler));
  }

  getAllSearch(searchTerm: string): Observable<Book[]> {
    return this.http.get<any>(`${environment.server}/search/${searchTerm}`).pipe(retry(3), map(res =>res['books']), catchError(this.errorHandler));
  }
